...
(defn chapter-url [ch img alt rel]
  (let [data-png (s/replace img ".svg" ".png")]
    (if ch
      (h/img-button (f/rel-url ch) img alt {:rel rel :data-png data-png})
...
(defn menu-chapter [book-id ch]
  (list
    ...
    (if (> (:chap-cnt ch) 1)
      [:li.chap (h/text-button (str ".#" (f/book-elem-id book-id)) (:num ch))])))
...
(defn number-markup [i ch]
  (if-let [num (verse-num i ch)]
    (list [:a {:href (str (f/rel-url ch) "#" num)} num] " ")
    ""))
